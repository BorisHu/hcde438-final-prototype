class Player extends Ship {
    ...
    controls() {
        ...
        if (keyIsDown(90) || keyIsDown(66) || dataarray[2] == 1) this.fire();
        
        // Movement (arrow keys)
        let diag = this.speed / sqrt(2);
        if (keyIsDown(RIGHT_ARROW) || keyIsDown(68) || dataarray[1] < 480) {
            if (keyIsDown(UP_ARROW) || keyIsDown(87) || dataarray[0] < 480) {
                this.vel = createVector(diag, -diag);
            } else if (keyIsDown(DOWN_ARROW) || keyIsDown(83) || dataarray[0] > 550) {
                this.vel = createVector(diag, diag);
            } else {
                this.vel = createVector(this.speed, 0);
            }
        } else if (keyIsDown(LEFT_ARROW) || keyIsDown(65) || dataarray[1] > 550) {
            if (keyIsDown(UP_ARROW) || keyIsDown(87) || dataarray[0] < 480) {
                this.vel = createVector(-diag, -diag);
            } else if (keyIsDown(DOWN_ARROW) || keyIsDown(83) || dataarray[0] > 550) {
                this.vel = createVector(-diag, diag);
        ...
        } else if (keyIsDown(UP_ARROW) || keyIsDown(87) || dataarray[0] < 480) {
            this.vel = createVector(0, -this.speed);
        } else {
            this.vel.mult(0);
        }
